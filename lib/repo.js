...
class PackageRepository {
    ...
    getModules(pkg) {
        return this._getModules(this.tlpobj(pkg));
    }

    getFontMaps(pkg) {
        return this._getFontMaps(this.tlpobj(pkg));
    }
    ...
    _getModules(tlpobj) {
        return this._getFiles(tlpobj)
            .map(fn => fn.match(/([^/]+)[.](sty|cls|tex|ltx)$/)?.[1]).filter(x => x);
    }
    ...
    async probe(pkgs, opts) {
        ...
        if (opts.pkg)
            pkgs = [].concat(...pkgs.map(pkg => this.scanPkg(pkg, opts)));
        // Prepare input
    ...
    scanPkg(pkg, opts) {
        ...
        this.db.recordModules(pkg, modules);
        return modules;
    }
...
class PackageInfoDB {
    ...
    async load() {
        ...
        /** @todo blatant overrides; should be in some data file */
        (this.pkgs['latex'] ??= {})
            .deps = ['l3kernel', 'l3backend', 'latex-fonts', 'ec'];
    }
...
export { PackageRepository, PackageInfoDB, RemotePackageRepository }
